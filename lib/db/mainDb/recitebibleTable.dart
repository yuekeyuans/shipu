...
class ReciteBibleTable {
  ...
  Future<bool> queryIsComplete(DateTime date) async {
    ...
    var result = Sqflite.firstIntValue(await db.query(TABLENAME, columns: ["count(1)"], where: "date = ? and iscomplete = ? ", whereArgs: [DateUtil.formatDate(date, format: DateFormats.y_mo_d), true.toString()]));
    return result != 0;
  }
  ...
  Future<List<ReciteBibleTable>> queryCurrentBookList(DateTime date) async {
    ...
    var db = await MainDb().db;
    var result = await db.query(TABLENAME, where: "date >= ?", whereArgs: [DateUtil.formatDate(ReciteBibleEntity.fromSp().startDate, format: DateFormats.y_mo_d)]);
  ...
  Future<void> generateData() async {
    ...
    for (var i = 0; i < times; i++) {
      ...
      var ids = <String>[];
      for (var j = minMax.first + i * length; j < minMax.first * (i + 1) && j <= minMax.last; i++) {
        ids.add(j.toString());
      }
      record.ids = ids;
  ...
  Future<ReciteBibleTable> queryByDay(DateTime date) async {
    ...
    var db = await MainDb().db;
    var result = await db.query(TABLENAME, where: "date = ?", whereArgs: [DateUtil.formatDate(date, format: DateFormats.y_mo_d)]);
  ...
  Future<bool> existDateRecord(DateTime date) async {
    var db = await MainDb().db;
    var result = Sqflite.firstIntValue(await db.query(TABLENAME, columns: ["count(*)"], where: "date = ?", whereArgs: [DateUtil.formatDate(date, format: DateFormats.y_mo_d)]));
    return result != 0;
  }
  ...
  Future<void> createDateRecord() async {
    ...
    //第一次开始
    if (DateUtil.formatDate(entity.startDate, format: DateFormats.y_mo_d) != DateUtil.formatDate(DateTime.now(), format: DateFormats.y_mo_d)) {
    ...
    var ids = <String>[];
    for (int i = 0; i < entity.verseOfDay && lastNumber + i <= minMax.last; i++) {
  ...
  Future<void> update() async {
    var db = await MainDb().db;
    await db.update(TABLENAME, toJson(), where: "date = ?", whereArgs: [DateUtil.formatDate(date, format: DateFormats.y_mo_d)]);
  }

  Future<void> deleteToday() async {
    var db = await MainDb().db;
    await db.delete(TABLENAME, where: "date = ?", whereArgs: [DateUtil.formatDate(DateTime.now(), format: DateFormats.y_mo_d)]);
  }
